...
import {
  ...
  User, Mail, Edit3, Save, Building2, Loader2,
} from "lucide-react";
import { getShipmentByOrderId, getWarehouses, getSuppliers } from "../../../services/shipmentService";
      ...
      <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }}
        ...
        <div className="p-8 text-center">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-teal-500 to-emerald-500 p-3.5 mx-auto mb-5 shadow-lg shadow-teal-500/25">
            <ShoppingBag className="w-full h-full text-[#1a1a1a]" />
          </div>
          <h2 className="text-2xl font-bold text-[#1a1a1a] mb-2">What do you receive?</h2>
          <p className="text-[#555555] text-sm italic">Select your category to get started</p>
        </div>
          ...
          {categoryOptions.map((cat) => (
            <button key={cat.id} onClick={() => setSelected(cat.id)}
              className={`p-4 rounded-xl border transition-all duration-200 text-center ${selected === cat.id
                ...
                : "bg-[#F9F7F2] border-[rgba(139,0,74,0.1)] hover:border-[rgba(139,0,74,0.2)] hover:bg-white/[0.05]"
                }`}>
              <span className="text-2xl block mb-1.5">{cat.icon}</span>
              <span className="block text-sm font-semibold text-center text-[#000000]">{cat.label}</span>
            </button>
          ))}
        </div>
...
/* ══════════════════════════════════════
   ...
   ══════════════════════════════════════ */
function OrderIdPage({ onSubmit }: { onSubmit: (id: string, shipment: any) => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onSubmit(trimmed, shipment as any);
    } catch {
    ...
    <div className="flex items-center justify-center min-h-[70vh]">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-[#1a1a1a]">Enter Order ID</h2>
          <p className="text-[#555555] text-sm mt-1">Enter your order ID to start tracking</p>
        </div>

        <div className="bg-white rounded-2xl p-6 border border-[rgba(139,0,74,0.08)] space-y-4">
          <div>
            <label className="text-[11px] text-[#555555] uppercase tracking-wider font-semibold mb-2 block">Order ID</label>
            <Input
              placeholder="e.g. ORD-2026-001"
              value={orderId}
              onChange={(e) => { setOrderId(e.target.value); setError(""); }}
              onKeyDown={(e) => e.key === "Enter" && handleSubmit()}
              className={`bg-white border-[#E5E5E5] text-[#1a1a1a] placeholder:text-[#888888] focus:border-teal-500/50 focus:ring-teal-500/20 h-12 text-center text-lg font-mono tracking-wider ${error ? "border-red-500/50" : ""}`}
            />
            {error && (
              <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-red-500 text-xs mt-2">{error}</motion.p>
            )}
          </div>
...
function WarehousesPage({ receiveCat }: { receiveCat: ReceiveCategory }) {
  ...
  useEffect(() => {
    setWLoading(true);
    getWarehouses(receiveCat)
      .then((data) => setAllWarehouses(data as any))
      .catch(() => setWError("Failed to load warehouses"))
      .finally(() => setWLoading(false));
  }, [receiveCat]);

  if (wLoading) return <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-teal-500" /></div>;
  if (wError) return <div className="text-center py-20 text-red-400">{wError}</div>;

  const matched = allWarehouses.filter((w) => w.category.includes(receiveCat));
  const others = allWarehouses.filter((w) => !w.category.includes(receiveCat));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-[#1a1a1a]">Connected Warehouses</h2>
            ...
            <Card key={w.id} delay={i * 0.06}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-teal-500 to-emerald-500 flex items-center justify-center shadow-md shadow-teal-500/20"><Package className="w-5 h-5 text-[#1a1a1a]" /></div>
                  <div>
...
function SuppliersPage({ receiveCat }: { receiveCat: ReceiveCategory }) {
  const [sortByRating, setSortByRating] = useState(true);
  const [allSuppliers, setAllSuppliers] = useState<SupplierContact[]>([]);
  const [sLoading, setSLoading] = useState(true);
  const [sError, setSError] = useState<string | null>(null);

  useEffect(() => {
    setSLoading(true);
    getSuppliers(receiveCat)
      .then((data) => setAllSuppliers(data as any))
      .catch(() => setSError("Failed to load suppliers"))
      .finally(() => setSLoading(false));
  }, [receiveCat]);

  if (sLoading) return <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-teal-500" /></div>;
  if (sError) return <div className="text-center py-20 text-red-400">{sError}</div>;

  const matched = allSuppliers.filter((s) => s.category.includes(receiveCat));
  const others = allSuppliers.filter((s) => !s.category.includes(receiveCat));
    ...
    <Card delay={idx * 0.06} className={faded ? "opacity-70" : ""}>
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className={`font-semibold ${faded ? "text-[#1a1a1a]" : "text-white"}`}>{s.name}</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-[#1a1a1a]">Suppliers for Your Order</h2>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-[#1a1a1a]">Receiver Profile</h2>
        <Button onClick={() => setPage("settings")} variant="outline" className="bg-white border-[rgba(139,0,74,0.1)] text-white hover:bg-[#F2EFE7] hover:text-white">
      ...
      <Card>
        <div className="flex items-center gap-4 mb-6 pb-6 border-b border-[rgba(139,0,74,0.1)]">
          ...
          <div>
            ...
            <span className="inline-block mt-1 text-[10px] font-semibold bg-teal-50 text-[#1F8A4C] border border-teal-200 rounded-full px-2 py-0.5">Receiver</span>
          </div>
        </div>
          ...
          {fields.map((f) => {
            ...
            return (
              <div key={f.label} className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] text-[#555555] uppercase tracking-wider font-semibold">{f.label}</p>
...
function ReceiverSettingsPage() {
  ...
  const inputStyles = "bg-white border-[rgba(139,0,74,0.1)] text-white placeholder:text-white/20 focus:border-teal-400/50 focus:ring-teal-400/20";
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t border-[rgba(139,0,74,0.1)]">
...
export default function ReceiverDashboard() {
  ...
  const handleOrderSubmit = (id: string, fetchedShipment: any) => {
    setOrderId(id);
